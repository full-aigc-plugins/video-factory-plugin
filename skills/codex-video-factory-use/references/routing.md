...
| 把目标和素材变成时间线 | `codex-video-factory-plan` | 不执行渲染 |
| 已批准粗剪或终版 | `codex-video-factory-run` | 无批准不得执行 |
| 技术与语义验收 | `codex-video-factory-judge` | 不改写计划 |
| 中断后的合法下一步 | `codex-video-factory-recover` | 不自动重试失败 |
