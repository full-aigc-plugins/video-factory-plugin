...
import { compileAssembly } from './ffmpeg-compiler.mjs';
import { collectMedia } from './media-collector.mjs';

const concatEscape = (path) => path.replaceAll("'", "'\\''");

export async function assembleVideo(segmentPaths, profile, destination, timeoutMs = 120000) {
  ...
  try {
    const spec = compileAssembly(listPath, profile, temp);
